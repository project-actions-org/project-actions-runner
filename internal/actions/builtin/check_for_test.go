...
func TestCheckForAction_Execute(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			config: map[string]interface{}{
				"check-for": "nonexistent-tool-12345",
			},
			wantErr: true,
		},
		{
			name: "nonexistent command with custom message",
			config: map[string]interface{}{
				"check-for": "nonexistent-tool-12345",
				"if-missing": "Please install the required tool",
			},
			wantErr: true,
		},
		{
			name:    "missing check-for field",
			config:  map[string]interface{}{},
			wantErr: true,
		},
		{
			name: "check-for with integer value",
			config: map[string]interface{}{
				"check-for": 123,
			},
			wantErr: true,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Note: Error messages go to stderr, not the captured buffer
			// The important part is that the action returns an error, which we check above
		})
	}
}
...
func TestCheckForAction_Validate(t *testing.T) {
	tests := []struct {
		...
		{
			name: "valid config with if-missing",
			config: map[string]interface{}{
				"check-for":  "docker",
		...
		{
			name: "only if-missing field",
			config: map[string]interface{}{
				"if-missing": "Some message",
			},
			wantErr: true,
		},
	}
